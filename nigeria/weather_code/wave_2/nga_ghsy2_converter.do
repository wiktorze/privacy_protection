...
* does
	* reads in Nigeria, wave 2 .csv files
...
* define paths
	loc root = "$data/weather_data/nigeria/wave_2/raw"
	loc export = "$data/weather_data/nigeria/wave_2/daily"
	loc logout = "$data/weather_data/nigeria/logs"

* open log
	log using "`logout'/nga_ghsy2_converter", replace
...
* define local with all sub-folders in it
	loc folderList : dir "`root'" dirs "GHSY2_rf*"
	...
	foreach file in `fileList' {
		...
		* save file
		customsave , idvar(hhid) filename("`dat'_`ext'_`sat'_daily.dta") ///
			path("`export'/`folder'") dofile(NGA_GHSY2_converter) user($user)
	}
}		
...
* define local with all sub-folders in it
	loc folderList : dir "`root'" dirs "GHSY2_t*"
