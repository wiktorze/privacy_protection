...
* does
	* reads in Nigeria, wave 3 .dta files with daily values
...
* define paths	
	loc root = "$data/weather_data/nigeria/wave_3/daily"
	loc export = "$data/weather_data/nigeria/wave_3/refined"
	loc logout = "$data/weather_data/nigeria/logs"

* open log	
	log using "`logout'/nga_ghsy3_weather", replace
	...
	foreach file in `fileList' {
		...
		* save file
		customsave , idvar(hhid) filename("`dat'_`ext'_`sat'_n.dta") ///
			path("`export'/`folder'") dofile(NGA_GHSY3_weather) user($user)
	}
}
