...
* open log
	log 	using 		"`logout'/mwi_ihps_weather", replace
	...
	* loop through each file in the above local
	foreach file in `fileList' {
		...
		* run the user written weather command - this takes a while
		weather rf_ , rain_data ini_month(1) fin_month(8) day_month(1) keep(y2_hhid)

		* save file
		customsave , idvar(y2_hhid) filename("`dat'_`ext'_`sat'.dta") ///
			path("`export'/`folder'") dofile(MWI_IHPS_weather) user($user)
	}
}
...
foreach folder of local folderList {
	...
	* loop through each file in the above local
	foreach file in `fileList' {
		...
		* run the user written weather command - this takes a while
		weather tmp_ , temperature_data growbase_low(10) growbase_high(30) ini_month(1) fin_month(8) day_month(1) keep(y2_hhid)
