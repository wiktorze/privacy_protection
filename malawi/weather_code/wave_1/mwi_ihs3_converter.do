...
* does
	* reads in Malawi IHS3, which we term wave 1, .csv files
...
* define paths
	loc 	root	= 	"$data/weather_data/malawi/wave_1/raw"
	loc 	export 	= 	"$data/weather_data/malawi/wave_1/daily"
...
foreach folder of local folderList {
	...
	* save file
	customsave , idvar(case_id) filename("`dat'_`ext'_`sat'_daily.dta") ///
		path("`export'/`folder'") dofile(MWI_IHS3_converter) user($user)
	}
}
