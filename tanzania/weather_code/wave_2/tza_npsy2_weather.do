...
* define paths	
	loc root = "$data/weather_data/tanzania/wave_2/daily"
	loc export = "$data/weather_data/tanzania/wave_2/refined"
	loc logout = "$data/weather_data/tanzania/logs"

* open log	
	log using "`logout'/tza_npsy2_weather", replace
	...
	foreach file in `fileList' {
		...
		* save file
		customsave , idvar(y2_hhid) filename("`dat'_`ext'_`sat'.dta") ///
			path("`export'/`folder'") dofile(TZA_NPSY2_weather) user($user)
	}
}
