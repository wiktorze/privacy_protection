...
* does
	* reads in Uganda, wave 3 .dta files with daily values
...
* define paths	
	loc root = "$data/weather_data/uganda/wave_3/daily"
	loc export = "$data/weather_data/uganda/wave_3/refined"
	loc logout = "$data/weather_data/uganda/logs"

* open log	
	log using "`logout'/uga_unpsy3_weather", replace
	...
	foreach file in `fileList' {
		...
		* save file
		customsave , idvar(hhid) filename("`dat'_`ext'_`sat'_n.dta") ///
			path("`export'/`folder'") dofile(UGA_UNPSY3_weather) user($user)
	}
}
