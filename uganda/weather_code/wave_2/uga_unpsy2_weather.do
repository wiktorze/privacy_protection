...
* define paths	
	loc root = "$data/weather_data/uganda/wave_2/daily"
	loc export = "$data/weather_data/uganda/wave_2/refined"
	...
	foreach file in `fileList' {
		...
		* save file
		customsave , idvar(hhid) filename("`dat'_`ext'_`sat'_n.dta") ///
			path("`export'/`folder'") dofile(UGA_UNPSY2_weather) user($user)
	}
}
